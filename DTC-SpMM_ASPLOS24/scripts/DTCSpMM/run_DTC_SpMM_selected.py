...
dataset_dir = repo_root / "dataset" / "selected_npz"
result_file = repo_root / "results" / "DTCSpMM_exe_time_and_throughput.csv"
result_file.parent.mkdir(parents=True, exist_ok=True)
os.environ["DTCSPMM_RESULT_FILE"] = str(result_file)

## Load matrix from files.
dataset = args.dataset
dset_name = dataset
path = dataset_dir / (dataset + ".npz")
matrix = DTC_dataset(str(path))
