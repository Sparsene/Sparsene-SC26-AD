...
__global__ void generate_tcoffset_id_atob(
    ...
    int *edgeList, int *tcblock_offset, uint8_t *tcblocktile_id,
		int *sparseatob, int *tcblock_pos_tmp, int *tcblock_nnz_tmp,
  ...
  if (num_blocks == 0) {
	...
	int *pos_ptr = tcblock_pos_tmp + block_start;
	int *tcblock_offset_ptr = tcblock_nnz_tmp + block_start;
  int *tcblock_offset_global_ptr = tcblock_offset + block_start;
	int *tcblock_nnz_ptr = tcblock_nnz_tmp + block_start;
  ...
  auto sparse_AToB = sparseatob + block_start * blockSize_w;
	if (num_blocks > 0) {
		tcblock_offset_ptr[0] = 0;
	}
	for (int i = 1; i < num_blocks; i++) {
		tcblock_offset_ptr[i] = tcblock_offset_ptr[i - 1] + tcblock_nnz_ptr[i - 1];
  }
  for (unsigned e_index = element_start; e_index < element_end; e_index++) {
    unsigned col = edgeToColumn[e_index]; // new col
...
void generate_tcoffset_id_atob_cuda(int *nodePointer, int *rowwindow_offset,
                                    ...
                                    int *edgeList, int *tcblock_offset,
																		...
																		int *tcblock_pos_tmp, int *tcblock_nnz_tmp,
  ...
  int window_count = num_row_windows;
	generate_tcoffset_id_atob<<<window_count, block_size>>>(
      nodePointer, rowwindow_offset, edgeToColumn, edgeToRow, edgeList,
			tcblock_offset, tcblock_tileid, sparseatob, tcblock_pos_tmp,
			tcblock_nnz_tmp, max_block, num_nodes, blockSize_h, blockSize_w,
			num_row_windows);
  cudaError_t error = cudaGetLastError();
  if (error != cudaSuccess) {
    printf("CUDA error: %s\n", cudaGetErrorString(error));
...
seg_sort_dequ(int *seg, int *edgeLists, int *nodepointer, int *edgetocol,
  ...
  auto tcblock_pos_tmp_tensor = torch::zeros({block_counter}, options_gpu);
  auto tcblock_nnz_tmp_tensor = torch::zeros({block_counter}, options_gpu);
  auto tcblock_offset = tcblock_offset_tensor.data<int>();
  auto sparse_AToX_index = sparse_AToX_index_tensor.data<int>();
  auto tcblock_pos_tmp = tcblock_pos_tmp_tensor.data<int>();
  auto tcblock_nnz_tmp = tcblock_nnz_tmp_tensor.data<int>();
  ...
  generate_tcoffset_id_atob_cuda(
	  ...
	  tcblock_offset + 1, tcblocktile_id, sparse_AToX_index, tcblock_pos_tmp,
	  tcblock_nnz_tmp, max_blocks, num_nodes, blockSize_h, blockSize_w,
	  rowwindow_num);
