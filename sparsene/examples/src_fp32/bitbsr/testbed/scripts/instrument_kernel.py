...
# Pipeline functions within BitBsrMainLoopOp that need instr params
...
# BitBsrMainLoopOp::f(int l, int r)
...
# dispatch() call within BitBsrMainLoopOp::f
DISPATCH_CALL_RE = re.compile(r"^(\s+)dispatch\(\);\s*$")

# Template f<>() calls for BitBsrMainLoopOp_v, blk_x_loop_v, blk_y_loop_v
TEMPLATE_CALL_RE = re.compile(
    r"^(\s+)(BitBsrMainLoopOp_v|bitbsr_spmm_kernel_blk_x_loop_v"
    r"|bitbsr_spmm_kernel_blk_y_loop_v)\.template f<>\("
)
...
def instrument(input_path: str, output_path: str) -> None:
    ...
    while i < len(lines):
        ...
        # ---- Track class context ----
        if "class bitbsr_spmm_kernel_blk_x_loop" in line or \
           "class bitbsr_spmm_kernel_blk_y_loop" in line:
            in_blk_loop = True
        ...
        # ---- Modify BitBsrMainLoopOp::f(int l, int r) definition ----
        ...
        # The top-level call is "bitbsr_spmm_kernel_blk_y_loop_v.template f<>(...)"
        ...
        if in_global_fn and line.strip() == "};" and not line.startswith(" "):
            ...
            # fp32 bitbsr: grid is (threadblock_num_x, threadblock_num_y), so
            # block_linear_idx = blockIdx.x * gridDim.y + blockIdx.y
            output.append("    // ---- Flush instrumentation to GMEM (auto-generated) ----\n")
            output.append("    {\n")
            output.append("        int _blk_lin = blockIdx.x * gridDim.y + blockIdx.y;\n")
