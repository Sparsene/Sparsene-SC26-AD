...
Usage:
  # Regenerate manifest from src_fp32/bitbsr/testbed plan_searcher
  python scripts/generate_manifest.py \
      --plan-searcher /path/to/src_fp32/bitbsr/testbed/scripts/plan_searcher.py \
...
def load_acc_function(plan_searcher_path: str):
    """Dynamically import bitbsr() from an arbitrary plan_searcher.py."""
    spec = importlib.util.spec_from_file_location("plan_searcher_ext", plan_searcher_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    # Try bitbsr() first, fall back to acc() for compatibility
    if hasattr(mod, 'bitbsr'):
        return mod.bitbsr
    return mod.acc
...
def main():
    ...
    # Load function from the specified plan_searcher
    print(f"Loading from {args.plan_searcher}")
