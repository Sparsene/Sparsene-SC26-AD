...
from perf_model import (
    ...
    BITBSR_SPMM_DEPENDENCIES,
    make_bitbsr_spmm_profiles,
    load_profiles,
)
...
def evaluate(plans: list[dict], profiles, deps) -> dict:
    ...
    metrics["ndcg"] = ndcg_at_k(sim_times, real_times)
...
def print_report(evaluation: dict, verbose: bool = False):
    ...
    print(f"\n{'Sim':>4s}  {'Real':>4s}  {'Label':<20s}  {'Pred II':>9s}  {'Kernel us':>10s}  ", end="")
    if verbose:
        print("Stages")
    else:
        print()
...
""")
    ...
    # Load profiles
    if args.profiles:
        profiles = load_profiles(args.profiles, defaults=make_bitbsr_spmm_profiles())
    ...
    # Evaluate
    evaluation = evaluate(plans, profiles, BITBSR_SPMM_DEPENDENCIES)
