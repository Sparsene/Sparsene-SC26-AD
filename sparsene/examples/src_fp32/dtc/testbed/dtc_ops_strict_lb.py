...
def dtc():
    dtc_program = NvOpProgram(
        ...
        gmem_inouts={
            ...
            #! Row Window is divided into multiple Row Blocks
            #! StrictLoadBalance_ThreadBlock_Offset: some ROW BLOCK is assigned to a Thread Block
            "slb_tb_offset": GmemInout(
                shape=Shape(VarlenShape("slb_block_dim_size")),
                name="slb_tb_offset",
                dtype="int",
                tensor_str="make_tensor(make_gmem_ptr(dslb_tb_offset), make_shape(slb_block_dim_size))",
                tensor_type_str="decltype(make_tensor(make_gmem_ptr((TO0*)nullptr), make_shape(VARLEN)))",
            ),
            #! every Row Block to Row Window IR
            "row_block_to_row_window_id": GmemInout(
                shape=Shape(VarlenShape("row_block_num")),
                name="row_block_to_row_window_id",
                dtype="int",
                tensor_str="make_tensor(make_gmem_ptr(drow_block_to_row_window_id), make_shape(row_block_num))",
                tensor_type_str="decltype(make_tensor(make_gmem_ptr((TO0*)nullptr), make_shape(VARLEN)))",
            ),
            #! every Row Block corresponding to some TC Block
            "row_block_offset": GmemInout(
                shape=Shape(VarlenShape("row_block_num")),
                name="row_block_offset",
                dtype="int",
                tensor_str="make_tensor(make_gmem_ptr(drow_block_offset), make_shape(row_block_num))",
                tensor_type_str="decltype(make_tensor(make_gmem_ptr((TO0*)nullptr), make_shape(VARLEN)))",
            )
        },
    )
    ...
    #! blk_x_loop body: the outer most for loop

    g2r_slb_tb_offset_load_op = NvOp(
        name="G2rSlbTbOffsetLoadOp",
        inputs=[
            NvOpInput(
                idx=0,
                name="slb_tb_offset",
                tensor=NvOpTensor(
                    ...
                    mem="gmem",
                    dtype="int",
                    source=dtc_program.gmem_tensor_ops["slb_tb_offset"].outputs[0],
                )
            ),
            NvOpInput(
                idx=1,
        ...
        impl=NvOpImpl(
            r"""
row_block_l(0) = slb_tb_offset(i);
row_block_r(0) = slb_tb_offset(i + 1);
"""
        ),
        mem_type=("g", "r"),
    )
    dtc_blk_x_loop_op.append(g2r_slb_tb_offset_load_op)

    #! for loop inside blk_x_loop body
    blk_x_row_block_loop_op = ForLoopNvOp(
        name="dtc_spmm_kernel_blk_x_row_block_loop",
        loop_l=NvOpInput(
            idx=0,
            name="l",
            tensor=NvOpTensor(
                ...
                mem="rmem",
                dtype="int",
                source=g2r_slb_tb_offset_load_op.outputs[0],
            )
        ),
        loop_r=NvOpInput(
            idx=1,
            ...
            tensor=NvOpTensor(
                ...
                mem="rmem",
                dtype="int",
                source=g2r_slb_tb_offset_load_op.outputs[1]
            )
        ),
        loop_result=[],
        body=NvOpSequence(),
    ...
    dtc_blk_x_loop_op.append(blk_x_row_block_loop_op)
    ...
    g2r_sparse_offset_load_op = NvOp(
        ...
        inputs=[
            NvOpInput(
                ...
                tensor=NvOpTensor(
                    ...
                    mem="gmem",
                    dtype="int",
                    source=dtc_program.gmem_tensor_ops["row_block_offset"].outputs[0],
                ),
            ),
            NvOpInput(
                idx=1,
                ...
                tensor=NvOpTensor(
                    ...
                    mem="rmem",
                    dtype="int",
                    source="_idx", #! from upper for loop, TODO: change to ForLoopOp.iter_args
                ),
            ),
        ],
        outputs=[
        ...
        impl=NvOpImpl(
            r"""
l(0) = row_block_offset(i);
r(0) = row_block_offset(i + 1);"""
        ),
        mem_type=("g", "r"),
    )
    #> change op to blk_x_row_block_loop_op
    # dtc_blk_x_loop_op.append(g2r_sparse_offset_load_op)
    blk_x_row_block_loop_op.append(g2r_sparse_offset_load_op)
    ...
    blk_x_row_block_loop_op.append(ZerosOp)
    ...
    blk_x_row_block_loop_op.append(dtc_main_loop_op)
...
for (int n_iter = 0; n_iter < get<1>(BLK_MMA_MNK{}); n_iter++) {
    ...
    g2r_row_block_to_row_window_id_load_op = NvOp(
        name="G2rRowBlockToRowWindowIdLoadOp",
        inputs=[
            NvOpInput(
                idx=0,
                name="row_block_to_row_window_id",
                tensor=NvOpTensor(
                    ...
                    mem="gmem",
                    dtype="int",
                    source=dtc_program.gmem_tensor_ops["row_block_to_row_window_id"].outputs[0],
                )
            ),
            NvOpInput(
                idx=1,
        ...
        impl=NvOpImpl(
            r"""
row_window_id(0) = row_block_to_row_window_id(row_block_id);
"""
        )
    )
    blk_x_row_block_loop_op.append(g2r_row_block_to_row_window_id_load_op)
        ...
        inputs=[
            ...
            NvOpInput(
                ...
                tensor=NvOpTensor(
                    ...
                    mem="rmem",
                    dtype="int",
                    source=g2r_row_block_to_row_window_id_load_op.outputs[0],
                )
            ),
            NvOpInput(
                idx=2,
            ...
            NvOpInput(
                ...
                tensor=NvOpTensor(
                    ...
                    mem="rmem",
                    dtype="int",
                    source="_l", #! from upper for loop, TODO: change to ForLoopOp.l
                )
            ),
            NvOpInput(
                idx=4,
                ...
                tensor=NvOpTensor(
                    ...
                    mem="rmem",
                    dtype="int",
                    source="_r", #! from upper for loop, TODO: change to ForLoopOp.r
                )
            )
        ],
        outputs=[
...
for (int i_tileN = 0; i_tileN < get<1>(BLK_MNK{}) / 8; i_tileN++) {
    ...
    if (i == row_block_l || i == row_block_r - 1) {
        atomicAdd(&C_val(col,       make_coord(row, row_window_id)), REGC(0, i_tileN));
        atomicAdd(&C_val(col + 1,   make_coord(row, row_window_id)), REGC(1, i_tileN));
        atomicAdd(&C_val(col,       make_coord(row + 8, row_window_id)), REGC(2, i_tileN));
        atomicAdd(&C_val(col + 1,   make_coord(row + 8, row_window_id)), REGC(3, i_tileN));
    } else {
        C_val(col,      make_coord(row, row_window_id)) = REGC(0, i_tileN);
        C_val(col + 1,  make_coord(row, row_window_id)) = REGC(1, i_tileN);
        C_val(col,      make_coord(row + 8, row_window_id)) = REGC(2, i_tileN);
        C_val(col + 1,  make_coord(row + 8, row_window_id)) = REGC(3, i_tileN);
    }
}"""
        ),
        mem_type=("r", "g"),
    ...
    #> change op to blk_x_row_block_loop_op 
    blk_x_row_block_loop_op.append(r2g_c_val_store_op)
...
if __name__ == "__main__":
    ...
    with open(script_dir / "kernel_strict_lb.inc", "w") as f:
