...
__global__ void acc_spmm_profiling_kernel(
    float *dB_val,
    float *dC_val,
    ...
    using G2sSparseIndexLoadOp_t = G2sSparseIndexLoadOp<NBUF, BLK_MNK, MMA_MNK, BLK_MMA_MNK, WARP_MNK>;
    ...
    G2sSparseIndexLoadOp_t G2sSparseIndexLoadOp_v{tid, lid, wid,
        G2sSparseIndexLoadOp_tensor_o0, nnz_block};
    ...
    for (int iter = 0; iter < k; iter++) {
      ...
      if (prof_mode == 0) {
        ...
        // Op 0: G2sSparseIndexLoadOp (cp.async sidx → smem)
        t0 = prof_clock();
        ...
        // Sandwich 0: Op0(G2sSparseIndexLoadOp) → Op4(G2sMatrixBLoadOp)
        // Direct: no other deps for Op4
        prof_drain();
        t0 = prof_clock();
        G2sSparseIndexLoadOp_v.template f<0>(
            val_sidxOp_v.template output<0>(), l + iter);
        ...
        // Need sidx in smem first (from sandwich 0, already there)
        prof_drain();
        t0 = prof_clock();
        G2sMatrixBLoadOp_v.template f<0>(
            B_valOp_v.template output<0>(),
...
void ncu_test(int threadblock_num_x, int threadblock_num_y, int thread_num,
    ...
    static const char* op_names[] = {
        "G2sSparseIndexLoadOp", "G2rSparseMcoOffLoadOp", "G2rSparseMcoMaskLoadOp",
    ...
    auto run_and_print = [&](int mode, const char* label) {
        ...
        acc_spmm_profiling_kernel<BLK_MNK, MMA_MNK, BLK_MMA_MNK, WARP_MNK,
...
// ---------------------------------------------------------------------------
// acc_spmm_test — same interface as host_program.cu
// ---------------------------------------------------------------------------

template <class dtypeA, class dtypeB, class dtypeC, class dtypeMask,
          int Tile_M, int Tile_N, int Tile_K, int Mma_M, int Mma_N, int Mma_K>
void acc_spmm_test(int M, int N, int K,
    ...
    CHECK_CUDA2(cudaMemcpy(dval_sidx, val_sidx, sizeof(int) * nnz_block * Tile_K, cudaMemcpyHostToDevice));
...
template void
acc_spmm_test<float, float, float, uint64_t, 16, 64, 8, 16, 8, 8>(int M, int N, int K,
