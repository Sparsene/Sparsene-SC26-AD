...
#
# Usage:
#   cd sparsene/examples/src_fp32/acc/testbed
...
#   --arch ARCH            CUDA arch (default: sm_90)
...
if [ -n "$PROFILES_PATH" ]; then
    ...
    nvcc $NVCC_FLAGS -o "$BUILD/acc_profiling" \
    ...
    # Use first GPU if --gpus specified
    GPU_PREFIX=""
    if [ -n "$GPUS" ]; then
        FIRST_GPU=$(echo "$GPUS" | tr ',-' '\n' | head -1)
        GPU_PREFIX="CUDA_VISIBLE_DEVICES=$FIRST_GPU"
    fi
    eval $GPU_PREFIX PROF_NBLOCKS=64 "$BUILD/acc_profiling" \
...
save_profiles(profiles, out_path, dependencies=ACC_SPMM_DEPENDENCIES,
