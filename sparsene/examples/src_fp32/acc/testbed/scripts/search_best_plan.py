...
_RUN_ENV_LOGGED = False
def run(cmd, cwd=None, quiet=False):
    global _RUN_ENV_LOGGED
    if not quiet:
        print(f"  $ {cmd[:120]}{'...' if len(cmd)>120 else ''}")
    if not _RUN_ENV_LOGGED:
        _RUN_ENV_LOGGED = True
        import os
        print(f"  [env] PATH={os.environ.get('PATH','?')[:100]}")
        print(f"  [env] LD_LIBRARY_PATH={os.environ.get('LD_LIBRARY_PATH','?')[:100]}")
...
def _parse_profiling_output(raw: str, out_path: Path):
    ...
    sys.path.insert(0, str(SCRIPTS))
    from perf_model import make_acc_spmm_profiles, save_profiles, ACC_SPMM_DEPENDENCIES

    ops = ["G2sSparseIndexLoadOp", "G2rSparseMcoOffLoadOp", "G2rSparseMcoMaskLoadOp",
    ...
    save_profiles(profiles, str(out_path), dependencies=ACC_SPMM_DEPENDENCIES,
...
def simulate_and_rank_all(output_dir: Path, manifest, top_n: int):
    ...
    from perf_model import (
        Pipeline, predict_steady_state_ii, ACC_SPMM_DEPENDENCIES,
        make_acc_spmm_profiles, load_profiles,
    )

    prof_path = output_dir / "profiles" / "op_profiles.json"
    profiles = (load_profiles(str(prof_path), defaults=make_acc_spmm_profiles())
                if prof_path.exists() else make_acc_spmm_profiles())

    results = []
    for entry in manifest:
        pipeline = Pipeline(stages=entry["stages"], shifts=entry["shifts"])
        try:
            ii = predict_steady_state_ii(pipeline, profiles, ACC_SPMM_DEPENDENCIES)
...
def compile_and_time(output_dir: Path, selected, args):
    ...
    Compilation runs in parallel (--compile-jobs workers), and timing runs
    on the main thread.  As soon as a binary is ready, it gets queued for
    timing so that GPU work overlaps with CPU compilation.

    A detailed event log is written to <output_dir>/timing/event_log.jsonl
    with per-plan compile_start/end and time_start/end timestamps.
    """
    import queue
    import threading
    ...
    def compile_one(pid):
        ...
        if r2.returncode == 0:
            # Verify binary was actually created and is executable
            if binary.exists() and binary.stat().st_size > 0:
                log_event(pid, "compile_end")
            else:
                log_event(pid, "compile_empty_binary")
                return
            ready_q.put((pid, binary))
        else:
    ...
    gpu_ids = getattr(args, 'gpu_ids', None) or [None]  # None = default GPU
    n_gpu = len(gpu_ids)
    ...
    def timing_worker(gpu_id):
        ...
        while True:
            try:
                ...
                # Check if compilation is done and queue is empty
            ...
            if not binary.exists():
                log_event(pid, "binary_missing", gpu=gpu_id)
                with results_lock:
                    timed_count[0] += 1
                continue
            bsize = binary.stat().st_size
            if bsize < 1000:
                log_event(pid, "binary_too_small", gpu=gpu_id, size=bsize)
                with results_lock:
                    timed_count[0] += 1
                binary.unlink(missing_ok=True)
                continue

            log_event(pid, "time_start", gpu=gpu_id, binary_size=bsize)
            cmd = f"{gpu_prefix} {binary} {kernel_args}".strip()
            # Use os.system + redirect to file (avoids subprocess.run issues)
            import os as _os
            rc = _os.system(f"{cmd} > {timing_file} 2>/dev/null")
            log_event(pid, "time_end", gpu=gpu_id, rc=rc)

            # Read back
            r_stdout = ""
            if timing_file.exists():
                r_stdout = timing_file.read_text()
            if timed_count[0] < 3 and (not r_stdout.strip() or rc != 0):
                print(f"  [DEBUG] pid={pid} gpu={gpu_id} rc={rc} stdout={len(r_stdout)}")
                print(f"  [DEBUG] cmd={cmd[:200]}")
            t_val = _parse_timing(r_stdout)
            if t_val is not None:
                with results_lock:
                    results[pid] = t_val
            ...
            # Keep binary for debugging (normally: binary.unlink(missing_ok=True))
            pass

            if n_done % 200 == 0:
                best_so_far = min(results.values()) if results else 0
                print(f"  Timed {n_done}/{total}, best so far: {best_so_far:.1f} µs")

    # Launch timing threads (1 per GPU)
    timing_threads = []
    ...
    if results:
        print(f"  Best: {min(results.values()):.1f} µs")
...
def report(output_dir: Path, manifest, selected, timing):
    ...
    print(f"  Best plan: {best_pid:04d}  ({best_time:.1f} µs)")
    ...
    for i, (pid, t) in enumerate(ranked[:5]):
        print(f"    #{i}: plan_{pid:04d}  {t:.1f} µs")
    print(f"{'='*60}")
...
def main():
    ...
    p.add_argument("--time-all", action="store_true",
                   help="Time ALL plans (ignore simulation ranking, use manifest order)")
    p.add_argument("--gpus", default=None,
                   help="GPU IDs for parallel timing, e.g. '4,5,6,7' or '4-7'. "
                        "Default: use CUDA_VISIBLE_DEVICES or GPU 0.")
    args = p.parse_args()
