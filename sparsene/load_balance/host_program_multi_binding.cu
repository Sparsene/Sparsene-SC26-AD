...
#include "kernel_multi_binding.inc"
...
void ncu_test(int threadblock_num_x,
    ...
    dtc_spmm_kernel_fp32_val_idx_bind_stage2<BLK_MNK, MMA_MNK, BLK_MMA_MNK, WARP_MNK,
        ...
        dsm_cycles,
        dblock_smid,
        K, M, Mo, N, nnz, nnz_block, threadblock_num_x);
}
    ...
    struct timeval t1, t2;
    {
        for (int i = 0; i < warmup_time; i++) {
            dtc_spmm_kernel_fp32_val_idx_bind_stage2<BLK_MNK, MMA_MNK, BLK_MMA_MNK, WARP_MNK,
                                                     tile_coo_val_size, tile_coo_val_size_no_pad,
                                                     tile_sidx_size>
              <<<grid, block>>>(dB_val, dC, drow_window_binding, drow_window_split, dval_coo_idx, 
                                dval_coo_off, dval_coo_val, dval_sidx,
...
void dtc_spmm_test_val_sidx_bind_reorder(int M,
    ...
    assert(N % TILE_B == 0);
    //! check load balance
    assert(row_window_binding.size() == row_window_blockDim_size);
    assert(row_window_split.size() == row_window_num);
    assert(row_window_val_soff.size() == row_window_blockDim_size + 1);
    ...
    CHECK_CUDA2(cudaMalloc(&dblock_smid, sizeof(int) * row_window_blockDim_size * (N / TILE_B)));
    CHECK_CUDA2(cudaMemset(dsm_cycles, 0, sizeof(unsigned long long) * 256));
    CHECK_CUDA2(cudaMemset(dblock_smid, -1, sizeof(int) * row_window_blockDim_size * (N / TILE_B)));

    CHECK_CUDA2(cudaMemcpy(dval_soff, row_window_val_soff.data(), sizeof(int) * row_window_val_soff.size(), cudaMemcpyHostToDevice));
    CHECK_CUDA2(cudaMemcpy(drow_window_binding, row_window_binding.data(), sizeof(int) * row_window_binding.size(), cudaMemcpyHostToDevice));
    CHECK_CUDA2(cudaMemcpy(drow_window_split, row_window_split.data(), sizeof(int) * row_window_split.size(), cudaMemcpyHostToDevice));
    ...
    //! update threadblock_num_x to row_window_blockDim_num
    ...
    if (!is_ncu_test) {
        repeat_test<TILE_B>(threadblock_num_x, threadblock_num_y, thread_num, M, N, K, Mo,
                            ...
                            dval_coo_idx, dval_coo_off, drow_window_binding, drow_window_split,
                            dC, dsm_cycles, dblock_smid, warmup_time, execute_time);
    }
    ...
    ncu_test<TILE_B>(threadblock_num_x, threadblock_num_y, thread_num, M, N, K, Mo, nnz_block,
                     ...
                     dval_coo_off, drow_window_binding, drow_window_split, dC, dsm_cycles,
                     dblock_smid);
