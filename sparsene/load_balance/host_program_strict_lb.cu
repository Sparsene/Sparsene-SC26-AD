...
void ncu_test(
    ...
    dtc_spmm_kernel_fp32_val_idx_bind_stage2<BLK_MNK, MMA_MNK, BLK_MMA_MNK, WARP_MNK, tile_coo_val_size, tile_coo_val_size_no_pad, tile_sidx_size><<<grid, block>>>(
        dB_val, 
        dC,
        drow_block_offset,
        ...
        dsm_cycles,
        dblock_smid,
        K, M, Mo, N, nnz, nnz_block, row_block_num, threadblock_num_x);
}
...
//! mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32
template <int TILE_B>
void dtc_spmm_test_val_sidx_bind_reorder(
    int M,
    int N,
    int K,
    int TCBlock_num,
    ...
    CHECK_CUDA2(cudaMalloc(&dC, sizeof(float) * (row_window_num * ROW_WINDOW_M) * N)); // C is row major (alloc more copy some)
    ...
    CHECK_CUDA2(cudaMalloc(&drow_block_to_row_window_id, sizeof(int) * row_block_to_row_window_id.size()));
    ...
    CHECK_CUDA2(cudaMalloc(&dblock_smid, sizeof(int) * slb_block_dim_size * (N / TILE_B)));
    CHECK_CUDA2(cudaMemset(dsm_cycles, 0, sizeof(unsigned long long) * 256));
    CHECK_CUDA2(cudaMemset(dblock_smid, -1, sizeof(int) * slb_block_dim_size * (N / TILE_B)));
    CHECK_CUDA2(cudaMemcpy(drow_block_offset, row_block_offset.data(), sizeof(int) * row_block_offset.size(), cudaMemcpyHostToDevice));
    CHECK_CUDA2(cudaMemcpy(drow_block_to_row_window_id, row_block_to_row_window_id.data(), sizeof(int) * row_block_to_row_window_id.size(), cudaMemcpyHostToDevice));
    CHECK_CUDA2(cudaMemcpy(dslb_tb_offset, slb_tb_offset.data(), sizeof(int) * slb_tb_offset.size(), cudaMemcpyHostToDevice));

    //! change to slb_block_dim_size
    ...
    if (!is_ncu_test) {
        repeat_test<TILE_B>(threadblock_num_x, threadblock_num_y, thread_num, M, N, K, Mo,
                            ...
                            dval_coo_idx, dval_coo_off, dC, row_block_num, drow_block_offset,
                            drow_block_to_row_window_id, dslb_tb_offset, dsm_cycles,
                            dblock_smid, warmup_time, execute_time);
    }
    ...
    ncu_test<TILE_B>(threadblock_num_x, threadblock_num_y, thread_num, M, N, K, Mo, nnz_block,
                     ...
                     dval_coo_off, dC, row_block_num, drow_block_offset,
                     drow_block_to_row_window_id, dslb_tb_offset, dsm_cycles,
                     dblock_smid);
...
void dtc_spmm_test_val_sidx_bind_reorder_runtime(int M,
                                                 int N,
                                                 int K,
                                                 int TCBlock_num,
                                                 int nnz,
                                                 ...
                                                 int execute_time) {
    if (tile_n == 64) {
        dtc_spmm_test_val_sidx_bind_reorder<64>(M,
                                                N,
                                                K,
                                                TCBlock_num,
                                                nnz,
    ...
    if (tile_n == 32) {
        dtc_spmm_test_val_sidx_bind_reorder<32>(M,
                                                N,
                                                K,
                                                TCBlock_num,
                                                nnz,
    ...
    if (tile_n == 16) {
        dtc_spmm_test_val_sidx_bind_reorder<16>(M,
                                                N,
                                                K,
                                                TCBlock_num,
                                                nnz,
        ...
        fprintf(stderr,
            "Unsupported tile_n=%d, only 16/32/64 is allowed in strict-lb test.\n",
            tile_n);
    exit(1);
}
...
void run_spmm_device_srcfp32_64_strict_lb(int M,
        ...
        const int threadblock_num_x = slb_block_dim_size;
