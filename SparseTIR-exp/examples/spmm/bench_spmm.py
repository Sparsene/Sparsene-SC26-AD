...
def load_mtx_graph(mtx_path: str):
    """Load a graph from a .mtx file.
    ...
    # Create a DGL graph
    g = dgl.graph(('csc', (matrix.indptr, matrix.indices, matrix.data)))
    return g


def get_default_config(nnz_per_row):
    """Generate default configuration based on matrix statistics
    
    Args:
        nnz_per_row: Average number of non-zeros per row
        
    Returns:
        tuple of (num_col_parts, bucket_sizes)
    """
    # Determine number of column partitions based on nnz_per_row
    if nnz_per_row > 500:
        num_col_parts = 16
    elif nnz_per_row > 200:
        num_col_parts = 8
    elif nnz_per_row > 50:
        num_col_parts = 4
    else:
    ...
    max_bucket = min(512, 2 ** (int(np.log2(nnz_per_row)) + 1))
    bucket_sizes = [2**i for i in range(int(np.log2(max_bucket)) + 1)]
    return num_col_parts, bucket_sizes
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mtx-file", type=str, help="path to MTX file")
    parser.add_argument("--implicit-unroll", "-i", action="store_true", help="use implicit unroll")
    args = parser.parse_args()
    
    if args.mtx_file:
        g = load_mtx_graph(args.mtx_file)
        # Calculate average nnz per row for auto-configuration
        avg_nnz_per_row = g.num_edges() / g.num_nodes()
        num_col_parts, bucket_sizes = get_default_config(avg_nnz_per_row)
        print(f"Auto-configured for matrix with {avg_nnz_per_row:.1f} nnz/row:")
        print(f"- Number of column partitions: {num_col_parts}")
        print(f"- Bucket sizes: {bucket_sizes}")
    else:
        g = get_dataset(args.dataset)
        num_col_parts = col_part_config[args.dataset]
        bucket_sizes = bucketing_config[args.dataset]
        
    print("num_col_parts", num_col_parts)
