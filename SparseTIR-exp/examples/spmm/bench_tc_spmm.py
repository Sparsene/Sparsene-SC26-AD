    ...
    with T.block("root"):
        for i, k, j in T.grid(16, 16, 16):
            with T.block("update"):
                ...
                C_frag[vii, vjj] = C_frag[vii, vjj] + T.cast(A_frag[vii, vkk] * B_frag[vkk, vjj], "float32")
...
def tcspmm(
    ...
    C = T.match_sparse_buffer(c, [IO, II, F], "float32")
    with T.sp_iter([IO, JO, II, JI, F], "SRSRS", "tcspmm") as [io, jo, ii, ji, f]:
        with T.init():
            C[io, ii, f] = T.float32(0)
        C[io, ii, f] = C[io, ii, f] + T.cast(A[io, jo, ii, ji] * B[ji, f], "float32")
...
def bench_tc_spmm(g: dgl.DGLHeteroGraph, x: th.Tensor, y_golden: th.Tensor, mma_shape_str: str):
    ...
    c_nd = tvm.nd.array(np.zeros(mb * tile_size * feat_size).astype("float32"), device=dev)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", "-d", type=str, default=None, help="dataset name")
    parser.add_argument("--mtx-file", "-f", type=str, default=None, help="path to .mtx file")
    parser.add_argument("--mma-shape", "-m", type=str, default="m16n16k16", help="MMA shape")
    args = parser.parse_args()
    
    if args.dataset is not None:
        g = get_dataset(args.dataset)
    elif args.mtx_file is not None:
        g = load_mtx_graph(args.mtx_file)
    else:
        raise ValueError("Either --dataset or --mtx-file must be specified")
        
    print(f"Graph loaded: {g.num_nodes()} nodes, {g.num_edges()} edges")
    
    for feat_size in [16, 32, 64, 128, 256, 512]:
