...
def main():
    ...
    file_name = project_dir / 'result' / 'Baseline' / 'spmm' / 'rode_spmm_f32_n512.csv'
    ...
    binary = project_dir / 'Baseline' / 'RoDe' / 'build' / 'eval' / 'eval_spmm_f32_n512'
    for index, row in df.iterrows():
        count += 1
        data_set = str(row['dataSet'])
        if data_set in ['adaptive', 'delaunay_n22', 'rgg_n_2_22_s0']:
            continue
